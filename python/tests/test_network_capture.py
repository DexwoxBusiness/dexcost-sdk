...
class _Resp:
    ...
    def __init__(
        self, status_code: int = 200, body_len: int = 0, content_type: str = "application/json"
    ):
        self.status_code = status_code
        self.headers = {"Content-Type": content_type, "Content-Length": str(body_len)}
...
def _task() -> Task:
    return Task(task_id=uuid.uuid4(), task_type="t", started_at=datetime.now(timezone.utc))
...
def test_bytes_land_on_task_counters():
    ...
    _handle_http_call(
        "https://api.uncataloged.com/v1/x",
        method="POST",
        request_headers={"Content-Type": "application/json"},
        request_body_len=120,
        response=_Resp(200, body_len=500),
        latency_ms=12,
    )
...
def test_uncataloged_above_threshold_emits_network_event():
    ...
    _handle_http_call(
        ...
        request_headers={},
        request_body_len=0,
        response=_Resp(200, body_len=200_000),
        latency_ms=40,
    )
...
def test_uncataloged_below_threshold_emits_no_event():
    ...
    set_current_task(task)
    _handle_http_call(
        "https://api.uncataloged.com/small",
        method="GET",
        request_headers={},
        request_body_len=0,
        response=_Resp(200, body_len=300),
        latency_ms=5,
    )
...
def test_uncataloged_error_emits_event_even_when_small():
    ...
    set_current_task(task)
    _handle_http_call(
        "https://api.uncataloged.com/fail",
        method="GET",
        request_headers={},
        request_body_len=0,
        response=_Resp(503, body_len=80),
        latency_ms=5,
    )
    events = get_recorded_events()
    assert len(events) == 1
...
def test_suppression_precedes_registered_domain_rate():
    task = _task()
    set_current_task(task)
    register_domain_rate("api.vendor.com", cost_usd="0.01")
    ...
    assert counters["bytes_in"] > 900
    assert counters["bytes_out"] > 40
...
def test_suppressed_call_records_bytes_but_no_network_event():
    ...
    with suppress_network_event():
        _handle_http_call(
            "https://api.suppressed-vendor.internal/v1/infer",
            method="POST",
            request_headers={},
            request_body_len=100,
            response=_Resp(200, body_len=300_000),
            latency_ms=900,
        )
...
def test_uncataloged_latency_trigger_emits_event():
    ...
    _handle_http_call(
        "https://api.uncataloged.com/latency",
        method="GET",
        request_headers={},
        request_body_len=0,
        response=_Resp(200, body_len=100),
        latency_ms=50,
    )
    events = get_recorded_events()
    assert len(events) == 1
...
def test_aiohttp_style_error_status_emits_event():
    ...
    _handle_http_call(
        "https://api.uncataloged.com/aiohttp-fail",
        method="GET",
        request_headers={},
        request_body_len=0,
        response=_AiohttpResp(),
        latency_ms=5,
    )
    events = get_recorded_events()
    assert len(events) == 1
