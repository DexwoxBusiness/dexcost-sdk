...
> **🔒 DECISIONS LOCKED 2026-05-20** — All 10 decisions in §5 + §5.1 below have been reviewed and locked. See [`decisions/2026-05-20-compute-foundation-decisions.md`](../decisions/2026-05-20-compute-foundation-decisions.md) for the locked answers, the strengthenings that emerged during review (K8s `k8s_node_aware` opt-in, single Phase 2 IMDS probe, per-runtime memory-unit conversion table, customer-facing positioning for #9/#10), and the cross-subsystem conventions extracted ([`conventions.md`](../conventions.md)). The decisions in this research doc remain as written for traceability — read the decisions log for the authoritative answers and any sharpenings.
...
1. **Pricing re-verification against live provider docs** — highest silent-error risk if skipped. This work lives in the **implementation plan** as the specific task "populate `compute_prices.json` with re-verified rates," NOT as a research-doc edit. The research doc remains a point-in-time snapshot; the spec doesn't depend on specific rate values (only on the *shape* of pricing — per-vCPU-s, per-GB-s, per-invocation), so re-editing the doc just to refresh numbers would create a stale-again window before code starts. The catalog refresh tooling (§3) is the canonical mechanism for keeping rates current.
