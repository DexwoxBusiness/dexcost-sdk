    ...
    it(testCase.name, () => {
      ...
      for (let index = 0; index < testCase.expected.length; index++) {
        expect(observed[index]).toMatchObject({
          ...
          unit: testCase.expected[index].unit ??
            defaultUnits[String(testCase.expected[index].metric)],
        ...
        expect(observed[index].dimensions).toEqual(testCase.expected[index].dimensions ?? []);
      }
    });
  }
